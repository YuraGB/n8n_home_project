...
(async () => {
  ...
  if (!arg) {
    console.error(
      "Please provide a JSON string or a path to a JSON file as an argument.",
    );
    process.exit(1);
  }
  try {
  ...
  // ------------------ //
  const latestResourceDataPromiseSettled =
    await defindAndFetchResourceData(formattedPostsData);
